#!/usr/bin/env python3
"""
Script de Ingestão V3.0 - Schema Cleanup (35 colunas)
Baseline: ingest_v2_7_4.py
Data: 13/12/2025

V3.0 Changes:
- ✅ Removed 15 unused columns (0% filled)
- ✅ Fixed process_calculo bug (field doesn't exist in schema)
- ✅ Schema: 50 → 35 columns (-30%)
"""
...
def main():
    ...
    print("=" * 80)
    print("📥 INGESTÃO V3.0 - Schema Cleanup (35 colunas)")
    print("=" * 80)
    ...
    json_files = [f for f in json_files if not f.stem.startswith("resumo") and not f.stem.startswith("resultados") and not f.stem.startswith("estatisticas")]
    ...
    # Query de INSERT V3.0 (35 colunas - 15 removidas + process_calculo removido)
    insert_query = """
        INSERT INTO esaj_detalhe_processos (
            ...
            valor_total_requisitado, saldo_final, data_saldo_final, origem_saldo_final, origem_data_saldo_final,
            ...
            %(valor_total_requisitado)s, %(saldo_final)s, %(data_saldo_final)s, %(origem_saldo_final)s, %(origem_data_saldo_final)s,
        ...
        DO UPDATE SET
            ...
            origem_saldo_final = EXCLUDED.origem_saldo_final,
            origem_data_saldo_final = EXCLUDED.origem_data_saldo_final,
    ...
    # Processar JSONs
    print(f"\n📋 Processando JSONs V3.0 (35 colunas)...")
    print("="*80)
    ...
    for json_file in tqdm(json_files, desc="Ingerindo"):
        try:
            ...
            # Log dos campos principais V3.0
            ...
            print(f"   Hab. Herdeiros: {data.get('habilitacao_herdeiros', False)}")

            # Preparar valores V3.0 (apenas 35 campos essenciais)
            ...
            valores = {
                ...
                # Dados Bancários (apenas essenciais)
                'banco': banco,
                'agencia': data.get('agencia'),
                'conta': data.get('conta'),

                # Valores Financeiros (apenas essenciais)
                ...
                'origem_saldo_final': data.get('origem_saldo_final'),
                'origem_data_saldo_final': data.get('origem_data_saldo_final'),
            ...
            # V3.0: 15 campos removidos do schema
            # V3.0: process_calculo removido (bug fix - não existe no schema)
    ...
    # Resumo
    print(f"\n" + "=" * 80)
    print(f"📊 RESUMO DA INGESTÃO V3.0")
    print("=" * 80)
    ...
    print("=" * 80)
    print()
    print("🎯 V3.0: Schema cleanup (50→35 cols) + process_calculo bug fixed")
